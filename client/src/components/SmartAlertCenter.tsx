  ...
  useEffect(() => {
    ...
    try {
        ...
        try {
          ...
          if (payload.type === 'new_alert' && payload.data) {
            const newAlert: Alert = {
              ...
              type: (payload.data.severity === 'CRITICAL' ? 'critical' : 'warning') as Alert['type'],
              title: payload.data.title || 'New Alert',
              message: payload.data.message || payload.data.title || '',
              description: payload.data.description || '',
              timestamp: new Date(payload.data.created_at || new Date()),
              read: false,
              priority: payload.data.severity === 'CRITICAL' ? 5 : 3,
            };
      ...
      wsRef.current.onerror = (error) => {
        ...
        console.error('Falling back to polling or demo alerts');
      };
