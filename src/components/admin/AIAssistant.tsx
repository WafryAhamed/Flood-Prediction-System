...
import { Send, Minimize2, Maximize2, X, Zap } from 'lucide-react';
    ...
    <AnimatePresence>
      <motion.div
        ...
        className={`fixed bg-white rounded-lg shadow-2xl z-50 flex flex-col border border-gray-300 ${
          isExpanded ? 'w-screen h-screen inset-0' : 'bottom-8 right-8 w-96 h-96'
        }`}
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-6 py-4 rounded-t-lg flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Zap size={20} />
            <h3 className="font-bold">AI Crisis Commander</h3>
          </div>
              ...
              >
                ...
                {message.action && (
                  <div className="mt-2 pt-2 border-t border-purple-500/30 flex gap-2">
                    <button className="text-xs px-2 py-1 bg-purple-500/30 rounded hover:bg-purple-500/50 transition-colors">
                      Execute
                    </button>
                    <span className="text-xs text-purple-100">{message.action}</span>
                  </div>
                )}
        ...
        <div className="px-4 py-3 border-b border-gray-200 grid grid-cols-2 gap-2">
          <button className="text-xs px-2 py-1 bg-purple-100 text-purple-600 rounded hover:bg-purple-200 transition-colors font-medium">
            📊 Risk Analysis
          </button>
          <button className="text-xs px-2 py-1 bg-blue-100 text-blue-600 rounded hover:bg-blue-200 transition-colors font-medium">
            🚨 Send Alert
          </button>
          <button className="text-xs px-2 py-1 bg-green-100 text-green-600 rounded hover:bg-green-200 transition-colors font-medium">
            📋 Generate Report
          </button>
          <button className="text-xs px-2 py-1 bg-orange-100 text-orange-600 rounded hover:bg-orange-200 transition-colors font-medium">
            🎯 Coordination
          </button>
        </div>
        ...
        <div className="p-4 border-t border-gray-200 bg-white rounded-b-lg flex gap-2">
          <textarea
            ...
            className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-purple-500 resize-none text-sm"
            rows={2}
            disabled={isLoading}
          />
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center h-fit"
          >
            <Send size={18} />
          </motion.button>
        </div>
        ...
        {/* Footer Info */}
        <div className="px-4 py-2 bg-purple-50 border-t border-purple-200 text-xs text-purple-600 rounded-b-lg">
