...
async def test_broadcast_flow():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10) as client:
        ...
        # Step 4: Retrieve via User API (non-admin)
        print("STEP 4: Retrieve Broadcast via User API (Public Access)")
        print("-" * 70)
        ...
        # Step 5: Verify SSE Event would be sent
        print("STEP 5: Verify SSE Event Stream (Check Infrastructure)")
        print("-" * 70)
        events_response = await client.get("/api/v1/integration/events")
        if events_response.status_code in [200, 401]:  # 401 expected if auth required
            ...
            print(f"   Note: Actual event delivery tests should use WebSocket connection or EventSource client")
        else:
        ...
        print("  5. SSE infrastructure responsive")
        print()
